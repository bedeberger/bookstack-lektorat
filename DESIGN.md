...
| [editor/notebook/page-head.css](public/css/editor/notebook/page-head.css) | Titel-Kopf des Beitrags im Notebook-Editor (`.page-head*`), nur publizistische Bücher: Leseansicht gesetzt wie im Ausgabeweg (Dachzeile · Schlagzeile · Lead), Edit-Modus als Felder mit Zeichen-Lineal (`progress-bar-wrap`/`progress-bar`-Pattern). Der Kopf im **Ausgabeweg** steht dagegen in `components/manuscript-stream.css` — er gehört zum server-gerenderten HTML, nicht zum Editor-Chrome. |
