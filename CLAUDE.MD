...
**Migration hinzufügen:** `CURRENT_SCHEMA_VERSION` erhöhen → `if (version < N)`-Block in `runMigrations()` ergänzen. Neue Tabellen als `CREATE TABLE IF NOT EXISTS` — keine Versionierung nötig.
