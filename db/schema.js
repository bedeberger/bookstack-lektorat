...
// A→B mentor ≡ B→A schuetzling. Für Dedup-Zwecke werden diese als identisch betrachtet.
...
// Entfernt alle Pages/Chapters, die in BookStack für dieses Buch nicht mehr
// existieren, plus deren abhängige Daten (Fehler-Historie, Stats, Chat-Sessions,
// Figuren-/Orte-Zuordnungen etc.). Umbenennungen werden NICHT gelöscht – die
...
function pruneStaleBookData(bookId, validPageIds, validChapterIds) {
  ...
  db.transaction(() => {
    if (stalePageIds.length > 0) {
      ...
      // Nicht löschen – User-kuratierte Daten, nur Verweise nullen (page_name/seite mit, damit reconcile sie nicht neu resettet)
    ...
    if (staleChapterIds.length > 0) {
      ...
      // Verweise in User-Daten nur nullen (kapitel bleibt in figure_scenes NOT NULL → nicht anfassen)
...
module.exports = { db, saveFigurenToDb, addFigurenBeziehungen, updateFigurenEvents, updateFigurenSoziogramm, saveZeitstrahlEvents, saveOrteToDb, reconcilePageIds, pruneStaleBookData, getUserToken, setUserToken, getAnyUserToken, getAllUserTokens, upsertUserLogin, getUser, updateUserSettings, saveCheckpoint, loadCheckpoint, deleteCheckpoint, insertJobRun, startJobRun, endJobRun, getBookSettings, getBookLocale, saveBookSettings, loadChapterExtractCache, saveChapterExtractCache, deleteChapterExtractCache, cleanupStuckJobRuns, getChapterFigures, cleanupDuplicateFiguren };
