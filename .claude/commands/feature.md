...
9. **Bei user-sichtbarem Feature:** `landing.feat<N>Title`/`Desc` in beiden Locales + `<N>` in `HELP_FEATURES` ([help-card.js](public/js/cards/help-card.js)) — SSoT für Landing + In-App-Hilfe. **Längenrahmen ist Pflicht** (DESIGN.md „Feature-Text (Landing + Hilfe)"): Titel 1–3 Wörter / max. 26 Zeichen, Beschreibung **160–200 Zeichen**, ein bis zwei Sätze — Satz 1 was es tut, Satz 2 der eine Unterschied. Keine Tabellen-/Routen-/Job-Namen, keine Pfade, keine **Why:**-Begründung. Die Texte stehen als Kacheln in einem Raster nebeneinander; ein längerer Text macht das Feature nicht wichtiger, sondern das Raster kaputt. Vor dem Melden nachzählen.
