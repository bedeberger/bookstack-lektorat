...
- **Bucheinstellungen** – Sprache/Region, Buchtyp (Roman, Kurzgeschichten, Krimi, …) und freier Kontext (Schauplatz, Epoche, Besonderheiten) werden in alle KI-Prompts eingebettet
...
**Konfigurierbar:**
- `locales` – Locale-Map (`de-CH`, `de-DE`, `en-US`, `en-GB`), jede mit:
  - `baseRules` – sprachspezifische Regeln (Schreibkonventionen, Fehlerentscheidungsregeln)
  - `systemPrompts` – Rollenformulierung pro KI-Einsatzbereich (`lektorat`, `buchbewertung`, `kapitelanalyse`, `figuren`, `stilkorrektur`, `chat`, `buchchat`, `orte`, `kontinuitaet`, `szenen`, `zeitstrahl`)
  - `stopwords` – Stoppwortliste für die Wortwolke
- `buchtypen` – Genre-Typen nach Sprachcode (`de`, `en`), je Key mit `label` (UI-Anzeige) und `zusatz` (KI-Kontext-Text, der in die `baseRules` aller Prompts eingebettet wird)
- `erklaerungRule` – globale Regel zur Fehlerfilterung (gilt für alle Locales)
- `defaultLocale` – Standard-Locale wenn kein Buch konfiguriert ist

**Per-Buch über die UI einstellbar** (in den Bucheinstellungen):
- **Buchtyp** – wählt einen vordefinierten Genre-Kontext aus `buchtypen`
- **Weiterer Kontext** – Freitext (max. 1000 Zeichen) für Schauplatz, Epoche, Besonderheiten etc.

Beide Werte werden automatisch in alle KI-Prompts dieses Buches eingebettet.

**Nicht konfigurierbar** (hartkodiert): JSON-Schemata, Feldnamen, Formatanweisungen.

Fehlt die Datei, wirft der Server beim Start einen Fehler – es gibt keine eingebauten Defaults.
