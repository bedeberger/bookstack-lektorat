...
//
// Zur Zeit genau ein Typ: `<figure>`. Der Zeiger darauf ist das `data-bid`, das
// lib/html-clean.js#ensureBlockIds am Schreib-Chokepoint ohnehin auf jeden Block
// setzt (`figure` steht in `_BID_BLOCK_SEL`). Es gibt also KEIN eigenes
// Anker-Attribut und keinen zusaetzlichen Write-Path — ein Querverweis auf eine
// Abbildung zeigt auf die Block-ID, die schon da ist.
//
...
/** Legendentext einer Abbildung als Klartext, auf eine Zeile normalisiert.
 *  Dient nur der Anzeige im Ziel-Picker und als Fallback-Beschriftung — die
 *  Wahrheit bleibt das HTML der Seite. */
function _caption(fig) {
  const cap = typeof fig.querySelector === 'function' ? fig.querySelector('figcaption') : null;
  if (!cap) return '';
...
export function collectFigureAnchors(root) {
  ...
  for (const fig of Array.from(root.querySelectorAll(FIGURE_SEL))) {
    ...
    out.push({ kind: 'figure', bid, caption: _caption(fig), ord: ord++ });
  }
  return out;
}
