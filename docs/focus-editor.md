...
| `focusCountWords` / `focusCountChars` (Live-Anzeige) | `_focusListeners` (ctx mit AbortController, Timer, Pointer-Flag, IO/MO, `progScrollTop`, …) |
...
| [card.js](../public/js/editor/focus/card.js) | **Nur Lifecycle:** `enterFocusMode`/`startFocusEdit`/`exitFocusMode`/`_focusInstall`/`_focusTeardown`/`_focusUpdateActive` (RAF-Wrapper) + `markFocusChrome`/`unmarkFocusChrome` (body-/Host-Klassen, Granularitäts-Klasse, `--focus-anchor`) |
