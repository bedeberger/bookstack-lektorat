...
async function loadContentForLink(link) {
  if (link.kind === 'page') {
    try {
      ...
      return {
        title: pd.name || '',
        html: _publicShareHtml(pd.html || '', link.token),
        toc: [],
      };
    } catch {
      return null;
...
// Lesezeit-Schaetzung). Gleiche Normalisierung wie routes/sync.js#htmlToText
// (Tags → Space, Whitespace kollabiert, getrimmt), damit der dem Leser gezeigte
// Umfang mit der App-Statistik konsistent bleibt.
