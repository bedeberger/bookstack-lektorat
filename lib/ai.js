  ...
  try { return JSON.parse(clean); } catch {
    ...
    if (m) {
      try { return JSON.parse(m[0]); } catch (e2) {
        ...
        throw new Error(`JSON-Parse fehlgeschlagen (${e2.message}). KI-Antwort (Anfang): ${preview}`);
      }
    }
    const preview = clean.length > 300 ? clean.slice(0, 300) + '…' : clean;
    throw new Error(`KI-Antwort enthält kein JSON-Objekt. KI-Antwort (Anfang): ${preview}`);
  }
}
