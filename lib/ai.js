...
//   ai/openai-compat.js — OpenAI-kompatibles Streaming (Text + Function-Calling).
//   ai/tool-translate.js— Uebersetzung kanonische Tool-Form <-> OpenAI-Function-Calling.
...
module.exports = {
  ...
  providerSupportsTools: config.providerSupportsTools,
  maxParallelCalls: config.maxParallelCalls,
  effectiveProviderClass: config.effectiveProviderClass,
  VALID_PROVIDERS: config.VALID_PROVIDERS,
  getContextConfigFor: config.getContextConfigFor,
  contextSafetyMargin: config.contextSafetyMargin,
  ...
  // Per-Call-Tier (Modell + Effort + Kosten-Label): normalizeTier fuer Aufrufer, die
  // ein Tier bauen oder auswerten; _resolveClaudeModel, um den effektiven Modellnamen
  // eines Calls zu bestimmen (Bepreisung in routes/jobs/shared/ai.js).
  normalizeTier: shared.normalizeTier,
  _resolveClaudeModel: config._resolveClaudeModel,
  // Preflight-Guard: passt der Prompt ins Kontextfenster? Vom Job-Chokepoint
  // (routes/jobs/shared/ai.js#aiCall) und defensiv von den lokalen Providern genutzt.
  estimatePromptTokens: shared.estimatePromptTokens,
  assertPromptFitsContext: shared.assertPromptFitsContext,
  CONTEXT_OVERFLOW_KEY: shared.CONTEXT_OVERFLOW_KEY,
};
