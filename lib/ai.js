...
async function _callOllama(messages, systemPrompt, onProgress, maxTokensOverride, signal) {
    ...
    const resp = await fetch(`${host}/api/chat`, {
      ...
      // format: 'json' zwingt Ollama via Grammar-Constrained Sampling zu validem JSON.
      //   Verhindert Markdown-Wrapper, Präambel-Text und – entscheidend für kleine Modelle –
      //   Wiederholungsloops, weil die JSON-Grammatik eindeutige Stop-Punkte vorgibt.
      body: JSON.stringify({ model, messages: allMessages, stream: true, format: 'json', options: { num_ctx, num_predict: maxTokens, temperature: parseFloat(process.env.OLLAMA_TEMPERATURE || '0.2'), think: false } }),
      signal,
    });
  ...
  try {
    resp = await fetch(`${host}/v1/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        // response_format: OpenAI-kompatibler JSON-Modus (LM Studio/llama.cpp).
        //   Erzwingt valides JSON via Grammar-Constrained Sampling – verhindert
        //   Markdown-Wrapper und Wiederholungsloops bei kleinen Modellen.
