...
async function callAI(userPrompt, systemPrompt, onProgress, maxTokensOverride, signal, provider) {
  ...
  if (provider === 'ollama') {
    return withOllamaLock(() => _callOllama(userPrompt, systemPrompt, onProgress, maxTokensOverride, signal));
  }
  if (provider === 'llama') {
    return withLlamaLock(() => _callLlama(userPrompt, systemPrompt, onProgress, maxTokensOverride, signal));
  }
  return _callClaude(userPrompt, systemPrompt, onProgress, maxTokensOverride, signal);
}

async function _callOllama(userPrompt, systemPrompt, onProgress, maxTokensOverride, signal) {
    ...
    const resp = await fetch(`${host}/api/chat`, {
      ...
      // num_ctx: Gesamtkontextfenster (Input + Output) – muss gross genug für beide sein.
      // num_predict: Output-Token-Limit (entspricht max_tokens bei Claude/Llama).
      body: JSON.stringify({ model, messages, stream: true, options: { num_ctx: globalMax, num_predict: maxTokens, temperature: parseFloat(process.env.OLLAMA_TEMPERATURE || '0.2'), think: false } }),
      signal,
    });
    if (!resp.ok) throw new Error(`Ollama ${resp.status}: ${await resp.text()}`);

    // Dient als Untergrenze – Ollama meldet bei KV-Cache-Treffer 0 oder nur User-Tokens.
    const estimatedTokIn = Math.ceil(messages.reduce((s, m) => s + (m.content?.length || 0), 0) / CHARS_PER_TOKEN);
...
async function _callLlama(userPrompt, systemPrompt, onProgress, maxTokensOverride, signal) {
...
async function _callClaude(userPrompt, systemPrompt, onProgress, maxTokensOverride, signal) {
...
module.exports = { callAI, parseJSON, CHARS_PER_TOKEN, MAX_TOKENS_OUT };
