...
async function _callOllama(messages, systemPrompt, onProgress, maxTokensOverride, signal, jsonSchema, temperatureOverride) {
    ...
    const temperature = (temperatureOverride != null && Number.isFinite(temperatureOverride))
      ? temperatureOverride
      : parseFloat(process.env.OLLAMA_TEMPERATURE || '0.2');
    const allMessages = [];
...
async function _callLlama(messages, systemPrompt, onProgress, maxTokensOverride, signal, jsonSchema, temperatureOverride) {
  ...
  const temperature = (temperatureOverride != null && Number.isFinite(temperatureOverride))
    ? temperatureOverride
    : parseFloat(process.env.LLAMA_TEMPERATURE || '0.1');
  const allMessages = [];
  if (systemPrompt) allMessages.push({ role: 'system', content: systemPrompt });
  allMessages.push(...messages);
...
module.exports = { callAI, callAIChat, callAIWithTools, parseJSON, chatTemperature, CHARS_PER_TOKEN, MAX_TOKENS_OUT };
