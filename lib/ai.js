...
// Input-Budget = Kontextfenster minus Output-Reservation minus Puffer. Untergrenze
// 4000 Tokens verhindert, dass bei Fehlkonfiguration (MODEL_TOKEN ≥ MODEL_CONTEXT)
// negative Werte durchrutschen.
const INPUT_BUDGET_TOKENS = Math.max(4000, MODEL_CONTEXT - MAX_TOKENS_OUT - CONTEXT_SAFETY_MARGIN);
const INPUT_BUDGET_CHARS  = INPUT_BUDGET_TOKENS * CHARS_PER_TOKEN;

logger.info(`AI-Budget: context=${MODEL_CONTEXT} out=${MAX_TOKENS_OUT} inputBudget=${INPUT_BUDGET_TOKENS} tok (~${INPUT_BUDGET_CHARS} chars)`);
...
async function callAIChat(messages, systemPrompt, onProgress, maxTokensOverride, signal, provider, jsonSchema, temperatureOverride) {
  ...
  if (provider === 'ollama') {
    return withOllamaLock(() => _callOllama(messages, systemPrompt, onProgress, maxTokensOverride, signal, jsonSchema, temperatureOverride));
  }
  if (provider === 'llama') {
    return withLlamaLock(() => _callLlama(messages, systemPrompt, onProgress, maxTokensOverride, signal, jsonSchema, temperatureOverride));
  }
...
function _combineSignals(userSignal, timeoutMs) {
  ...
  const timer = setTimeout(() => ctrl.abort(new Error(`Claude-Timeout nach ${timeoutMs}ms`)), timeoutMs);
  const cleanup = () => {
    clearTimeout(timer);
...
async function _callClaude(messages, systemPrompt, onProgress, maxTokensOverride, signal) {
    ...
    const { signal: combinedSignal, cleanup } = _combineSignals(signal, CLAUDE_TIMEOUT_MS);
    try {
