...
async function _callOllama(userPrompt, systemPrompt, onProgress, maxTokensOverride, signal) {
    ...
    const resp = await fetch(`${host}/api/chat`, {
      ...
      body: JSON.stringify({ model, messages, stream: true, options: { num_ctx, num_predict: maxTokens, temperature: parseFloat(process.env.OLLAMA_TEMPERATURE || '0.2'), think: false } }),
      signal,
    });
