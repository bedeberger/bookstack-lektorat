...
async function _renderDropCapParagraph(doc, runs, font) {
  ...
  // Cap für N Zeilen sizen + greedy-fit; gibt {dropSize, dropW, lastFit, linesUsed} zurück.
  const fitForLines = (N) => {
    doc.font('heading');
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (h <= targetH) { acc = trial; lastFit = trial; } else break;
    }
    const fitH = lastFit ? doc.heightOfString(lastFit, { width: wrapW, lineGap }) : 0;
    const linesUsed = Math.max(1, Math.round(fitH / lineSpacing));
    return { dropSize, dropW, wrapW, lastFit, linesUsed };
  };

  let res = fitForLines(3);
  // Wenn nur ≤2 Zeilen tatsächlich rendern → Cap auf 2 verkleinern + neu fitten.
  if (res.linesUsed < 3) {
    res = fitForLines(2);
  }
  ...
  doc.x = startX;
  doc.y = startY + linesUsed * lineSpacing;
