...
//   1. dropLines = 3 (Industriestandard, optisch ausgewogen).
//   2. dropSize aus Ascender-Verhältnissen abgeleitet — Cap-em-Top = startY,
//      Cap-Baseline = Body-Zeile-N-Baseline. Ergibt bündigen Cap-Top mit
//      erster Body-Zeile und exaktes Ende auf Baseline der N-ten Zeile,
//      ohne Überlappung. Skaliert mit konfiguriertem font.body.lineHeight.
//   3. fullText greedy nach Worten zerteilen, solange heightOfString ≤ N
//      Zeilen bei reduzierter Breite (wrapW = fullW − dropW − gap).
//   4. Pass 1 rendert den Slice an indentX, Pass 2 den Rest am Margin.
//
...
async function _renderDropCapParagraph(doc, runs, font) {
  ...
  // Ascender-Ratio (em-Top → Baseline) aus pdfkit-Fontmetriken.
  ...
  // capAsc = bodyAsc + (N-1)·lineSpacing  ⇒  Cap-Baseline = Baseline der N-ten Body-Zeile
  const capAsc = bodyAsc + (dropLines - 1) * lineSpacing;
  const dropSize = capAsc / capAscRatio;
  doc.fontSize(dropSize).fillColor('#000000');
...
async function _renderBlock(doc, block, ctx) {
  ...
  if (block.kind === 'blockquote') {
    ...
    const onPageAdded = (newPage) => {
      const prevBottom = newPage.height - newPage.margins.bottom;
      segments.push({ pageIdx: segPageIdx, y0: segY0, y1: prevBottom });
      segPageIdx += 1;
      newPage.margins.left = indentedLeft;
      segY0 = newPage.margins.top;
    };
