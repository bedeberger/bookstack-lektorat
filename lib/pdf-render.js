...
async function _renderDropCapParagraph(doc, runs, font) {
  ...
  // Deterministisches Line-Counting via widthOfString (heightOfString respektiert
  // den lineGap-Optionswert in einigen pdfkit-Versionen nicht zuverlässig).
  ...
  const fitForLines = (N) => {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (countLines(trial, wrapW) <= N) { acc = trial; lastFit = trial; }
      else break;
    }
    const linesUsed = countLines(lastFit, wrapW);
