...
function _xrefModules() {
  if (!_modPromise) {
    _modPromise = Promise.all([
      _esm('public', 'js', 'xrefs', 'xref-html.js'),
      _esm('public', 'js', 'xrefs', 'xref-anchor.js'),
      _esm('public', 'js', 'xrefs', 'xref-number.js'),
      _esm('public', 'js', 'xrefs', 'xref-format.js'),
...
async function _anchorsFromGroups(groups) {
  const { collectFigureAnchors } = await _xrefModules();
  const out = [];
  for (const g of Array.isArray(groups) ? groups : []) {
    for (const x of g.pages || []) {
      const html = x?.pd?.html;
      if (typeof html !== 'string' || html.indexOf('<figure') === -1) continue;
      const { document } = parseHTML(`<!doctype html><html><body><div id="r">${html}</div></body></html>`);
      const root = document.getElementById('r');
      if (!root) continue;
      for (const a of collectFigureAnchors(root)) {
...
async function buildXrefContext({ bookId, groups = [], chapterLabels = null, userEmail = null } = {}) {
  ...
  const { chapter, figure } = buildXrefNumbers({ chapters, anchors, chapterLabels });
  ...
  return { lang, figureNumbering, chapter, figure };
}

// ── Anwenden ─────────────────────────────────────────────────────────────────

function _entryFor(ctx, kind, target) {
  const map = kind === 'chapter' ? ctx?.chapter : kind === 'figure' ? ctx?.figure : null;
  if (!map) return null;
  return (map instanceof Map ? map.get(String(target)) : map[String(target)]) || null;
}

/** Verweistexte setzen und Abbildungslegenden nummerieren — in EINEM DOM-Pass.
 *
...
async function applyXrefsInHtml(html, ctx = {}) {
  ...
  const {
    ...
    FIGURE_SEL, formatXref, figureCaptionPrefix,
  } = await _xrefModules();
  ...
  if (wantCaptions) {
    for (const fig of Array.from(root.querySelectorAll(FIGURE_SEL))) {
      const bid = String(fig.getAttribute('data-bid') || '').trim().toLowerCase();
      const entry = _entryFor(ctx, 'figure', bid);
      const prefix = figureCaptionPrefix(entry?.number, ctx.lang);
      if (!prefix) continue;
      const cap = fig.querySelector('figcaption');
      if (!cap) continue;
      // Praefix wird vorangestellt, nicht eingerechnet: die Legende gehoert dem
