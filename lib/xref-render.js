...
async function applyXrefsInHtml(html, ctx = {}) {
  ...
  const {
    ...
    FIGURE_ANCHOR_SEL, TABLE_ANCHOR_SEL, formatXref, captionPrefix,
  } = await _xrefModules();

  // Billiger Vorab-Test, bevor ein DOM gebaut wird. Die Literale kommen aus der
  // SSoT, nicht aus einer Kopie.
  //
  // JE TYP EIN GATE, und beide Typen muessen hier stehen: Abbildungen und
  // Tabellen tragen eigene Schalter (ein Fachbuch nummeriert oft Tabellen und
  // keine Abbildungen), und eine Seite kann ausschliesslich Tabellen fuehren.
  // Ein gemeinsamer Gate auf `figureNumbering && '<figure>'` liesse die
  // Tabellenbeschriftung genau dann ungenummert, waehrend das Verzeichnis
  // (lib/anchor-directory.js, haengt nur an `number`) sie schon auflistet —
  // der Verweis zeigte auf eine Zahl, die im Dokument nirgends steht.
  const hasXrefs = html.indexOf(XREF_ATTR_ID) !== -1;
  const wantFigCaptions = !!ctx.figureNumbering && html.indexOf('<figure') !== -1;
  const wantTableCaptions = !!ctx.tableNumbering && html.indexOf('<table') !== -1;
  const wantCaptions = wantFigCaptions || wantTableCaptions;
  ...
  if (wantCaptions) {
    ...
    const CAPTIONED = [];
    if (wantFigCaptions) CAPTIONED.push({ sel: FIGURE_ANCHOR_SEL, kind: 'figure', capSel: 'figcaption' });
    if (wantTableCaptions) CAPTIONED.push({ sel: TABLE_ANCHOR_SEL, kind: 'table', capSel: 'caption' });
