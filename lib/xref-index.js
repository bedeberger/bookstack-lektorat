...
//
// Parsing-Logik wird NICHT dupliziert: `collectXrefs`/`collectAnchors`
...
// Schnell-Ausschluss vor dem Parsen: beide Anker-Typen. Mit nur `<figure` wuerde
// eine Seite, die ausschliesslich Tabellen traegt, nie indiziert — der Verweis
// darauf faende sein Ziel dann nicht.
const ANCHOR_HINTS = ['<figure', '<table'];
...
async function reindexPageXrefs(pageId, html) {
  ...
  const mayHaveAnchors = ANCHOR_HINTS.some(h => src.indexOf(h) !== -1);
  ...
  const { collectXrefs, xrefsByTarget, collectAnchors } = await _xrefModules();
  ...
  const anchors = mayHaveAnchors
    ? replacePageAnchors(pid, collectAnchors(root))
