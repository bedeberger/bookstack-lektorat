...
// CJS-SPIEGEL des Befund-Vokabulars aus public/js/prompts/textsorten.js. Ein
// Import ist nicht moeglich (ESM dort, CJS hier) und ein dynamisches `import()`
// wuerde dieses Modul unrein machen — genau die Eigenschaft, wegen der die
// Zaehlerei hier ohne Datenbank und ohne Prompt-Stack testbar ist. Drift ist
// durch tests/unit/struktur-vokabular-drift.test.mjs gegated.
const URTEILE = ['traegt', 'lueckenhaft', 'verfehlt'];
const W_FRAGEN = ['wer', 'was', 'wann', 'wo', 'wie', 'warum'];
/** Die Status mit Handlungsbedarf — nur sie zaehlen als Formluecke. */
const STATUS_OFFEN = ['teilweise', 'fehlt'];
...
function summarizeStrukturChecks(checks, pages, { scope = 'book' } = {}) {
  ...
  for (const c of relevant) {
    ...
    for (const r of c.result?.regeln || []) {
      if (!STATUS_OFFEN.includes(r?.status)) continue;
      const key = `${ts}|${r.nr}`;
...
module.exports = { summarizeStrukturChecks, URTEILE, W_FRAGEN, STATUS_OFFEN, URTEIL_RANK };
