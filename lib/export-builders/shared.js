...
async function prepareCitations(bundle, opts = {}) {
  ...
  if (!bib) return { groups: bundle?.groups || [], bib: null, showBibliography: false, notes: false };
  ...
  // Gruppe des Kapitels.
  const notes = bib.notesMode === 'endnotes';
  const groups = notes
    ? (await buildEndnotes(bundle.groups, bib)).groups
    : await resolveCitesInGroups(bundle.groups, bib);
...
module.exports = { escXml, resolveTitle, resolveSlug, chapterDepth, buildChaptersById, ancestorInSet, prepareCitations, notesTitleFor };
