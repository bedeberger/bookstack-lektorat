...
// Paragraphen gemappt. Tabellen werden als Fliesstext durchgereicht. Eingefügte
...
const { resolveTitle, chapterDepth, buildChaptersById, ancestorInSet, prepareCitations, notesTitleFor } = require('./shared');
...
function _headParagraphs(p, cfg, ctx, { titleLevel = null } = {}) {
  const out = [];
  ...
  if (titleLevel != null) out.push(_chapterHeading(null, headline.pageTitle(p), titleLevel, cfg, ctx, true));
  ...
  groups.forEach((g, gi) => {
    ...
    if (ch && g.pages.length > 1) {
      ...
      g.pages.forEach((x) => {
        ...
        if (nested || headline.needsOwnHead(x.p)) {
          out.push(..._headParagraphs(x.p, cfg, ctx, { titleLevel: Math.min(2, level + 1) }));
        }
        out.push(..._htmlToParagraphs(x.pd.html, cfg, ctx));
      });
    } else {
      const x = g.pages[0];
      const name = ch ? ch.name : headline.pageTitle(x.p);
      out.push(_chapterHeading(label, name, level, cfg, ctx, gi === 0));
      // Mit Kapitel bleibt dessen Name die Ueberschrift (Ressort), der Beitrag
      // bekommt seinen Kopf mit eigener Ueberschrift darunter. Ohne Kapitel IST
      // die Ueberschrift oben schon der Beitragstitel — dann nur Dachzeile und
      // Lead, und die Dachzeile steht hier notgedrungen darunter statt darueber.
      if (headline.needsOwnHead(x.p)) {
        out.push(..._headParagraphs(x.p, cfg, ctx, { titleLevel: ch ? Math.min(2, level + 1) : null }));
      }
      out.push(..._htmlToParagraphs(x.pd.html, cfg, ctx));
    }
