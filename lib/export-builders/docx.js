...
function _buildBodyParagraphs(bundle, cfg, ctx) {
  ...
  groups.forEach((g, gi) => {
    ...
    if (g.notes && g.notes.length) out.push(..._endnoteParagraphs(g.notes, ctx.bib, cfg, ctx));
  });
  return out;
}
...
async function buildDocxProfile(bundle, opts = {}) {
  ...
  const prepared = await prepareCitations(bundle, { ...opts, bibliography: bib, scope: opts.scope || bundle.scope });
  const citeResolved = bib ? { ...bundle, groups: prepared.groups } : bundle;
  ...
  const ctx = { lang, surname, titleKeyword, title, suppressIndent: true, bib };
