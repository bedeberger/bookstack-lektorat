...
// openai-compat ist per Default 'local' (llama.cpp/vLLM & Co); der Admin-Schalter
// `ai.openai-compat.cloud` stuft gehostete Frontier-APIs (z.B. Kimi/Moonshot,
// OpenAI) auf 'cloud' hoch. Per-Call gelesen → Admin-Änderung greift ohne Restart.
// Bewusst KEIN Ersatz fuer `=== 'claude'`-Gates: Claude-API-Features (Prompt-Caching,
// Web-Search-Tool, Tiered Routing, Kontinuitäts-Verify) haengen weiter am Provider,
// nicht an der Klasse.
function providerClass(provider) {
  const p = VALID_PROVIDERS.has(provider) ? provider : 'claude';
  if (p === 'claude') return 'cloud';
  if (p === 'openai-compat' && appSettings.get('ai.openai-compat.cloud') === true) return 'cloud';
  return 'local';
}
...
function _resolveClaudeEffort(override) {
  const raw = override != null && String(override).trim() !== '' ? override : getContext().claudeEffort;
...
module.exports = {
  VALID_PROVIDERS, resolveProvider, providerClass,
