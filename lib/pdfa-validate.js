...
function _run(args, input) {
  return new Promise((resolve, reject) => {
    ...
    // Spawn-Fehler (ENOENT, EACCES, ENOMEM …): execFile leitet sie via callback
    // weiter, aber das `error`-Event auf dem ChildProcess feuert oft VOR dem
    // Callback. Ohne Listener crasht der Prozess.
    child.on('error', () => { /* von execFile-Callback abgehandelt */ });

    if (input && child.stdin) {
      // Wenn veraPDF gar nicht startet (oder sofort stirbt), ist stdin ein
      // bereits geschlossenes Pipe und `write` triggert EPIPE — ohne 'error'-
      // Listener emittiert Node ein UnhandledError-Event und der Prozess
      // crasht. Wir schlucken Stream-Errors hier; der Callback oben liefert
      // den eigentlichen Spawn-Fehler.
      child.stdin.on('error', () => {});
      try {
        child.stdin.end(input);
      } catch {
        // EPIPE / ERR_STREAM_DESTROYED: child ist tot, callback klärt das.
      }
    }
  });
}
