...
let _escPromise = null;
function _escapeModule() {
  if (!_escPromise) _escPromise = _esm('public', 'js', 'utils', 'escape.js');
  return _escPromise;
}
...
async function buildBibliography({ bookId, pageIds = null, citations: citationsIn = null, userEmail = null } = {}) {
  ...
  return { enabled, inBlog, title, titleHtml: escHtml(title), style, lang, scope, numbers, suffixes, sourcesById, entries };
}
