...
function _enforceWidowOrphan(doc, runs, opts) {
...
async function _renderBlock(doc, block, ctx) {
  const { font, indent = 0, token, imageCache, dropCapHint, firstParaHint, bodyFirstLineIndentPt = 0, hangingIndentPt = 0, textRole = 'body', columns = 1, columnGap = 0, hyphenate = null, widowOrphanControl = true, skipWidowOrphan = false, dpiWarnThreshold = 0, dpiWarnings = null } = ctx;
  ...
  if (block.kind === 'paragraph') {
    ...
    if (widowOrphanControl && !skipWidowOrphan && columns === 1) {
      _enforceWidowOrphan(doc, block.runs, runOpts);
    }
  ...
  if (block.kind === 'blockquote') {
    ...
    for (const sub of block.blocks) {
      await _renderBlock(doc, sub, { ...ctx, dropCapHint: { pending: false }, firstParaHint: { pending: false }, bodyFirstLineIndentPt: 0, skipWidowOrphan: true });
    }
