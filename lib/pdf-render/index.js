...
async function renderPdfBuffer({ book, groups, profile, coverBuf, authorImageBuf, token, lang, scope = 'book', chapter, page, meta, bibliography = null }) {
  ...
  const useNotes = bibliography?.notesMode === 'endnotes';
  const srcGroups = !bibliography
    ? groups
    : useNotes
      ? (await buildEndnotes(groups, bibliography)).groups
      : await resolveCitesInGroups(groups, bibliography);
