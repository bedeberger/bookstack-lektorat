...
function _renderRuns(doc, rawRuns, opts) {
  ...
  const single = (opts.columns || 1) === 1;
  // Runs mit Notenmarker MUESSEN durch den eigenen Layouter, auch wenn sie
  // linksbuendig gesetzt werden (Listen): nur er hat einen Per-Zeilen-Hook, mit
  // dem die Note ihren Platz am Seitenfuss reservieren kann. Der pdfkit-Pfad
  // unten paginiert selbst und snapshottet dabei `maxY` — eine mitten in der
  // `continued`-Kette aufgebaute Reserve bliebe dort unsichtbar, und der Text
  // liefe in den Apparat.
  const hasNote = !!opts.footnotes && rawRuns.some(r => Number.isInteger(r && r.noteId));
  if (single && ((opts.align || 'justify') === 'justify' || hasNote)) {
