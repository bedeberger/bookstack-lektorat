'use strict';
// Fund-Index der Belege: liest die Chips aus dem gespeicherten Seiten-HTML und
...
// den Marker-Attributnamen? Buecher ohne Belege kosten damit nichts — bei
...
/** Belege einer Seite neu indizieren.
 *  Gibt die Anzahl indizierter Quellen zurueck (0 wenn die Seite keine Belege
...
async function reindexPageCitations(pageId, html) {
  ...
  if (typeof html !== 'string' || html.indexOf(MARKER_HINT) === -1) {
    // Keine Belege im HTML. Aufraeumen nur, wenn ueberhaupt Zeilen existieren —
  ...
  const { collectCites, citationsFromCites } = await _citeHtmlModule();
  const { document } = parseHTML(`<!doctype html><html><body><div id="r">${html}</div></body></html>`);
  const root = document.getElementById('r');
  if (!root) return 0;
  return replacePageCitations(pid, citationsFromCites(collectCites(root)));
}
